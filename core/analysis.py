...
def Calculate_BRISQUEDIFF(src: str, dst: str):
...
# Function > Image RMSE
def Calculate_RMSE(src: str, dst: str):
    imgA = cv2.imread(src)
    imgB = cv2.imread(dst)
...
# Function > Image MSE
def Calculate_MSE(src: str, dst: str):
    imgA = cv2.imread(src)
    imgB = cv2.imread(dst)
...
# Function > Image PSNR
def Calculate_PSNR(src: str, dst: str):
...
def Image_Quality_Information(src: str, dst: str):
    quality_information = []
...
def Generate_Analysis_Results(src: str, dst: str):
...
def convert_csv(formatted: [], savepath: str) -> None:
...
def Generate_CSV(sender: str) -> None:
